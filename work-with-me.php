<?php
$pageTitle = 'Work with me — Grace Pariser';
$pageDescription = 'Consultancy for business owners. A back-office subscription for HR consultants.';
$activeNav = 'work';
require __DIR__ . '/includes/header.php';
?>

<section class="hero wrap">
    <h1>Work with me</h1>
    <div class="prose lede">
        <p>Consultancy for business owners. A back-office subscription for HR consultants.</p>
    </div>
</section>

<section class="section wrap">
    <p class="section-label">For business owners: HR On Call</p>
    <div class="prose">
        <p>Retainer support, project work, and one-off advice on TUPE, settlement agreements, disciplinary and grievance processes, and bespoke document drafting. I work with businesses directly and through HR intermediaries.</p>
    </div>
    <div class="cta-row">
        <a class="btn" href="/contact.php">Enquire</a>
    </div>
</section>

<section class="section wrap">
    <p class="section-label">For HR consultants</p>

    <div class="article-card">
        <h3>The HR Vault</h3>
        <p>The back office of an HR consultancy, in one subscription. The document library, the builders that draft for you, the calculators and toolkits, and 30 minutes of expert advice a month, so you spend your time on clients instead of paperwork. Monthly, rolling, cancel whenever you like.</p>
        <div class="cta-row" style="margin-top:0.75rem;">
            <a class="btn-quiet" href="https://thehrvault.co.uk" target="_blank" rel="noopener">Explore The HR Vault</a>
        </div>
    </div>
    <div class="article-card">
        <h3>Practice Hub</h3>
        <p>Practice management built for how HR consultants actually work.</p>
        <div class="cta-row" style="margin-top:0.75rem;">
            <a class="btn-quiet" href="https://practice-hub.co.uk" target="_blank" rel="noopener">Explore Practice Hub</a>
        </div>
    </div>
    <div class="article-card">
        <h3>Pop + Pixel</h3>
        <p>Websites for HR consultants.</p>
        <div class="cta-row" style="margin-top:0.75rem;">
            <a class="btn-quiet" href="https://popandpixel.co.uk" target="_blank" rel="noopener">Explore Pop + Pixel</a>
        </div>
    </div>
</section>

<?php require __DIR__ . '/includes/footer.php'; ?>
